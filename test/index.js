...
var publishedPackagePath = path_1.join(storeMainDr, 'packages', values.depPackage, values.depPackageVersion);
var publishedPackage2Path = path_1.join(storeMainDr, 'packages', values.depPackage2, values.depPackage2Version);
var checkExists = function (path) {
    return assert_1.doesNotThrow(function () { return fs.accessSync(path); }, path + ' does not exist');
};
var checkNotExists = function (path) {
    return assert_1.throws(function () { return fs.accessSync(path); }, path + ' exists');
};
...
describe('Yalc package manager', function () {
    ...
    describe('Package publish', function () {
        ...
        it('handles "files:" manifest entry correctly', function () {
            ...
            checkNotExists(path_1.join(publishedPackagePath, 'folder2/nested/file2.txt'));
            checkNotExists(path_1.join(publishedPackagePath, 'test'));
        });
        it('does not respect .npmignore, if package.json "files" present', function () {
            checkExists(path_1.join(publishedPackagePath, 'src', 'file-npm-ignored.txt'));
        });
        ...
        it('Adds signature to package.json version', function () {
            ...
            var versionLength = values.depPackageVersion.length + shortSignatureLength + 1;
        ...
        describe('signature consistency', function () {
            ...
            for (var tries = 1; tries <= 5; tries++) {
                it("should have a consistent signature after every publish (attempt " + tries + ")", function () {
                    var sigFileName = path_1.join(publishedPackagePath, 'yalc.sig');
    ...
    describe('Add package', function () {
        ...
        it('places yalc.lock correct info about file', function () {
            ...
            assert_1.deepEqual(lockFile.packages, (_a = {},
        ...
        it('updates package.json', function () {
            ...
            assert_1.deepEqual(pkg.dependencies, (_a = {},
        ...
        it('create and updates installations file', function () {
            ...
            assert_1.deepEqual(installations, (_a = {},
                _a[values.depPackage] = [projectDir],
                _a));
        });
    });
    describe('Update package', function () {
        var innterNodeModulesFile = path_1.join(projectDir, 'node_modules', values.depPackage, 'node_modules/file.txt');
        before(function () {
