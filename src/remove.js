...
var isYalcFileAddress = function(address, name) {
  var regExp = new RegExp(
    'file|link:' + _1.values.yalcPackagesFolder + '/' + name
  )
    ...
    return __generator(this, function(_a) {
      switch (_a.label) {
        case 0:
          ...
          if (packages.length) {
            packages.forEach(function(packageName) {
              ...
              if (lockFileConfig.packages[name]) {
                if (
                  !version ||
                  version === lockFileConfig.packages[name].version
                ) {
          ...
          if (
            !Object.keys(lockFileConfig.packages).length &&
            !options.retreat
          ) {
          ...
          installationsToRemove = packagesToRemove.map(function(name) {
            return {
              name: name,
              version: '',
              path: workingDir
          ...
          packagesToRemove.forEach(function(name) {
            if (!options.retreat) {
              fs.removeSync(
                path_1.join(workingDir, _1.values.yalcPackagesFolder, name)
              )
            }
          })
