...
function checkManifest(options) {
  var findLocalDepsInManifest = function(manifestPath) {
    ...
    var addresMatch = new RegExp(
      '^(file|link):(.\\/)?\\' + _1.values.yalcPackagesFolder + '\\/'
    )
    var findDeps = function(depsMap) {
      return Object.keys(depsMap).filter(function(name) {
        return depsMap[name].match(addresMatch)
      })
    }
    var localDeps = findDeps(pkg.dependencies || {}).concat(
